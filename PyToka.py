...
try:
    ...
    showerror("Error","Permission Error: Cannot Write or Read settings.json")
    root.destroy()
else:
    json_data = k[0]
    my_language = k[1]
    ...
    class PyToka(Window):
        def __init__(self, string):
            ...
            self.line_text = Text(self,width=5, height=1600, spacing3=6, bd=0, takefocus=0, state="disabled",
                                    cursor="arrow", font=(json_data["theme"]["edit_font"], 11), spacing1=2, spacing2=7, bg="white")
        ...
        def _license(self):
            ...
            with open("LICENSE.txt", "r", encoding="utf-8") as f:
                self.mpl2 = f.read()
            self.license.text = Text(self.license, width=11451, height=19198,font=("Segoe UI",16))
